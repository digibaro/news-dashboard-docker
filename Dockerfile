...
COPY go.mod go.sum ./
RUN go mod download
COPY main.go feeds.go panels.go main_test.go config.yaml.default ./
COPY web ./web
# the image is only built when vet and the unit tests pass (including a check of config.yaml.default)
RUN go vet ./... && go test -count=1 ./...
ARG VERSION=dev
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w -X main.version=${VERSION}" -o /out/nieuwsdashboard .
